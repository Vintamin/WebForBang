...
export default {
  ...
  created() {
    this.getpersonRealTimeList(this.stationName);
  },
  methods: {
    getpersonRealTimeList: function(stationName, name) {
      this.$axios({
        ...
        .then(res => {
          ...
          // console.log(this.personRealTimeData)
          ...
          this.personRealTimeData.forEach((item, index) => {
            item.INDEX_CODE = index + 1;
          })
    ...
    rowClick(row) {
      this.$axios("api/persorientatio/Installationsite/" + row, {
        params: {
